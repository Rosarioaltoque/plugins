...
class BpfbBinder {
	...
	function js_load_scripts () {
		...
		wp_localize_script('bpfb_interface_script', 'l10nBpfb', array(
			...
			'paste_geo_url' => __('Paste geo URL here', 'bpfb'),
		));
	}
	...
	/**
	 * Handles geo preview requests.
	 */
	function ajax_preview_geo () {
		$url = $_POST['data'];
		$warning = __('There has been an error processing your request', 'bpfb');
		$response = $url ? __('Processing...', 'bpfb') : $warning;
		$ret = wp_oembed_get($url);
	...
	function ajax_update_activity_contents () {
		...
		if (@$_POST['data']['alta_actividad_latitud']) {
			$bpfb_code = $codec->create_geo_tag($_POST['data']['alta_actividad_latitud'],$_POST['data']['alta_actividad_longitud']);
		}
		...
		if ($bpfb_code) {
			...
			if (@$_POST['data']['alta_actividad_latitud']) {
				bp_activity_update_meta($aid, 'geo:lat', $_POST['data']['alta_actividad_latitud']);
				bp_activity_update_meta($aid, 'geo:long', $_POST['data']['alta_actividad_longitud']);
				bp_activity_update_meta($aid, 'georss:point', $_POST['data']['alta_actividad_latitud'].' '.$_POST['data']['alta_actividad_longitud']);
			}
		}
		if ($aid) {
			ob_start();
	...
	function add_hooks () {
		...
		add_action('wp_ajax_bpfb_preview_geo', array($this, 'ajax_preview_geo'));
