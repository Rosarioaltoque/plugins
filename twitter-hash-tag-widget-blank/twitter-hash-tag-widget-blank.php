<?php
/*
Plugin Name: Twitter Hash Tag Blank Widget
...
Version: 1.1
        ...
        Twitter Hash Tag Blank Widget is released under the GNU General Public License (GPL)
        http://www.gnu.org/licenses/gpl-2.0.txt
*/

class WP_Widget_Twitter_Hash_Tag_Blank extends WP_Widget {

	function WP_Widget_Twitter_Hash_Tag_Blank() {
		$widget_ops = array('description' => __( 'Real time Twitter hash tag blank following' ) );
		$this->WP_Widget('twitter_hash_tag_blank', __( 'Twitter Hash Tag Blank' ), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
        ...
        echo $before_widget;
        if ( $title )
            echo $before_title . "<a href='http://search.twitter.com/search?q=%23$hashtag' target='_blank'>$title</a>" . $after_title;
		...
		if ( is_wp_error($raw_response) ) {
			...
			$output .= get_option('twitter_hash_tag_blank_cache');
		} else {
			...
			$output = "<ul class='twitter-hash-tag-blank-widget'>\n";
			foreach ( $response['results'] as $result ) {
				...
				if ( $images )
					$output .= "<a href='$user_url' target='_blank'><img src='$image' alt='$user' /></a>";
				$output .= "<a href='$user_url' target='_blank'>$user</a>: $text <a href='$source_url' target='_blank'>&raquo;</a></li>\n";
			}
			$output .= "<li class='view-all'><a href='http://search.twitter.com/search?q=%23$hashtag' target='_blank'>" . __('View All') . "</a></li>\n";
			$output .= "</ul>\n";
		}

		if ( ! is_wp_error($raw_response) )
			update_option('twitter_hash_tag_blank_cache', $output);
...
function wp_widget_twitter_hash_tag_blank_init() {
	add_option('twitter_hash_tag_blank_cache', '');
}

add_action('widgets_init', create_function('', 'return register_widget("WP_Widget_Twitter_Hash_Tag_Blank");'));

register_activation_hook(__FILE__, 'wp_widget_twitter_hash_tag_blank_init');
